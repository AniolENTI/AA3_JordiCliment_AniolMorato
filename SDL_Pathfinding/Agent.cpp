...
vector<Vector2D> Agent::calculateBreathFirstSearch(Vector2D agentPosition, Vector2D targetPosition)
{
	queue<Vector2D> frontierFIFO;
	frontierFIFO.push(agentPosition);
	frontierCount.clear();

	unordered_map<Vector2D, Vector2D> cameFrom;
	cameFrom[agentPosition] = NULL;

	vector<Vector2D> neighbors;
	Vector2D current, next;
	bool visited;

	while (!frontierFIFO.empty()) 
	{
		current = frontierFIFO.front();
		frontierFIFO.pop();
		...
		if (current == targetPosition)
		{
			...
			path.points.push_back(agentPosition);
		...
		for (unsigned int i = 0; i < neighbors.size(); i++)
		{
			...
			for (unsigned int j = 0; j < cameFrom.size(); j++)
			{
